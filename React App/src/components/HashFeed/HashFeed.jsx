...
import NewHashtag from '../Hashtags/NewHashtag';
import Hashtags from './../Hashtags/Hashtags';
import './HashFeed.css';
import RespNavbar from '../Navbar/RespNavbar'
import '../Feed/Feed.css';

export default function HashFeed() {
  ...
  const [allowedLocation, setAllowedLocation] = useState(false)
        ...
        () => {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                axios({
                    method: "GET",
                    url:
                      url + "/api/feed/"+username+"/"+position.coords.latitude+"/"+position.coords.longitude,
                    headers: {
        ...
        () => {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                axios({
                    method: "GET",
                    url:
                      url + "/api/hashfeed/"+username+"/"+position.coords.latitude+"/"+position.coords.longitude,
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: `${token}`,
                    },
    ...
    <>
      {screenReady&&allowedLocation ? 
        <div className="fluid-container mw-100 feed-page">
        ...
        <div className="col-lg-3 col-md-10 col-sm-10 hashtags">
          <Hashtags/>
          <NewHashtag/>
        </div> 
          </div>        
