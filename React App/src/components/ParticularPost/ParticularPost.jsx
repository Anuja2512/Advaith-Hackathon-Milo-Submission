        ...
        () => {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                axios({
                    method: "GET",
                    url:
                      url + "/api/post/"+id+"/"+position.coords.latitude+"/"+position.coords.longitude,
                    headers: {
        ...
        <div className="fluid-container g-0 mw-100 feed-page">
          ...
          {showBigNavbar || window.innerWidth>1000 ?null:<RespNavbar />}
            ...
            {/* <div className="col post-cont"> */}
              ...
              { screenReady ? <Post id={id} data={post} /> : null}
              { screenReady ? <AddComment id={id} data={post}  /> : null}
              { screenReady ? <Comments id={id} data={post}  /> : null }
              </div>                    
            {/* </div>     */}
            </div> 
          </div>
          ...
          <div className="container container-fluid" style={{top: "3%", position: "sticky", margin: "5% auto"}}>
              <div className="container container-fluid light-bg" style={{margin: "5% auto"}}><img className="img-fluid" src="/images/4.svg" alt="" /></div>
            </div>
          </div> 
            </div>        
    </div>
